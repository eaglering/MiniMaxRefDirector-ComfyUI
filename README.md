...
**v3.1.8** — 面向 MiniMax H3 Reference-to-Video 的「导演式」多段分镜工作流。
...
- **导演向快捷控件**：分镜编辑器一键多选运镜 / 景别 / 构图 / 微表情，随生成请求注入提示词；运镜支持时间线级全局默认与分段继承 / 覆盖，空选择即交由模型自由书写。
...
- **优化主体参考绑定**：
  - 绑定链路收敛到后端统一服务 `/h3/build_subject_bindings`：subject 定义（subject_definition）、保留分析（retention_analysis）与图片 / 音频 / 视频媒体清单由后端一次生成，前端（画布与分镜编辑器）直接消费结果，替代原先各处的本地拼装，多段复用与结果一致性更稳。
  - 描述区输入 `@` 唤起主体选择器，快速插入 `<@主体>` 引用并自动完成绑定；引用类子主体按类型自动命名（图片 1 / 视频 1 / 音频 1）并回收复用，音频引用自动回写父主体的音轨绑定（audioRef）。
  - 保留描述支持段级覆盖：编辑器中针对某段手动指定的 retention 优先于主体卡片默认值。
  - 生成前对「被 @ 提及 / 已添加但未定义」的主体给出未匹配提醒，避免绑定静默丢失。
- **提示词导演向快速功能**：
  - 分镜编辑弹窗新增导演控制列：**运镜 / 景别 / 构图 / 微表情** 均为一键多选快捷标签（chip），点选即随该段生成请求注入；音频段自动隐藏运镜与表演控件。
  - 运镜支持时间线级「全局默认」（设置菜单），未单独设置的段自动跟随；任一段可点选覆盖为自定义，或一键清除回到「跟随默认 / 自由」，空选择即交由模型自由书写，避免技术标签污染正文。
  - 微表情内置表演提示词库（抿唇 / 转瞬微笑 / 视线游移等），支持从本地 JSON 导入自定义词库扩充；选中后由 LLM 按各镜头情绪织入克制写实的表演描写，中文输出下自然意译为中文。
  - 生成结果支持一键复制完整 H3 提示词（带剪贴板反馈）。
...
- **motion-context 支持 `context_latent` 跨段衔接**：`MiniMax Ref Guide` 新增可选 `context_latent`（LATENT）输入并**优先于 `prev_tail`**：
  - 传入上一段最终 joint latent（KSampler 输出 / `MiniMax Ref Combine` 的 `context_latent` 输出 / H3 Motion Context LoadLatent）时，直接解码其视频流尾部像素帧作为 motion context（跳过「先落盘 mp4 再解码」的有损往返，抑制多次循环噪点累积），并把含音频流的原 latent 透传给 H3 Motion Context（音频直切，免 `audio_vae` 二次重建）；
  - `prev_tail` 像素帧路径完整保留作兜底；未接 `context_latent` 时行为与旧版一致；
  - `MiniMax Ref Combine` 新增 `context_latent`（LATENT）输出：透传本段最终 joint latent（仅引用不修改），供工作流循环接回下一段 Guide 的 `context_latent`。
- **片段级「降噪」开关**：分镜编辑器「引导强度」控件下方新增「降噪」开关与可折叠参数（起始α / 末端α / 斜坡 / 随机种子）。开启后 Guide 在 motion-context 路径内嵌调用 ComfyUI-H3-Context-Noise 注入锥形噪声（`tail_frames` 自动取该段引导强度吸附后的合法 H3 run）：
  - latent 路径：`MiniMaxH3ContextLatentTaperNoise`（只注视频流尾帧，音频流不动）；
