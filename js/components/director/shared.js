...
const STYLES = `
.mrd-pr-wrapper{font-family:ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif;display:flex;flex-direction:column;gap:8px;width:100%;height:100%;box-sizing:border-box;padding-bottom:4px}
.mrd-pr-gp-mount{width:100%;box-sizing:border-box;padding:6px 0 0;flex-shrink:0}
.mrd-pr-wrapper.drag-active{outline:2px dashed #888;background:rgba(255,255,255,0.05);border-radius:6px}
.mrd-pr-toolbar{display:flex;justify-content:space-between;align-items:center;padding:2px 0px;flex-wrap:wrap;gap:6px}
.mrd-pr-actions{display:flex;gap:6px;flex-wrap:wrap}
.mrd-pr-btn{background:#222;color:#e0e0e0;border:1px solid #111;border-radius:4px;padding:6px 12px;font-size:11px;font-weight:500;cursor:pointer;display:flex;align-items:center;gap:6px;transition:all 0.2s ease}
.mrd-pr-btn:hover:not(:disabled){background:#333;border-color:#555}
.mrd-pr-btn.toggle-on{background:#e0e0e0;border-color:transparent;color:#1c222d}
.mrd-pr-btn.toggle-on:hover:not(:disabled){background:#f0f0f0}
.mrd-pr-btn-danger:hover:not(:disabled){background:#4a1515;border-color:#cc4444;color:#ffaaaa}
.mrd-pr-icon-btn{padding:5px 8px;min-width:30px;justify-content:center;flex:0 0 auto}
.mrd-pr-icon-btn svg{display:block}
.mrd-pr-canvas{background:#2a2a2a;cursor:pointer;width:100%;outline:none;display:block}
.mrd-pr-prop-container{display:flex;flex-direction:column;width:100%;flex:none;min-height:40px}
.mrd-pr-prompt-wrapper{position:relative;display:flex;flex-direction:column;width:100%;height:auto;min-height:120px;background:#222;border:1px solid #111;border-radius:6px;box-sizing:border-box;transition:border-color 0.2s ease,opacity 0.2s ease;overflow:hidden}
.mrd-pr-prompt-wrapper.focus-active{border-color:#888}
.mrd-pr-wrapper.has-focus .mrd-pr-prompt-wrapper:not(.focus-active),.mrd-pr-wrapper:has(.mrd-pr-prompt-wrapper.focus-active) .mrd-pr-prompt-wrapper:not(.focus-active){opacity:0.65}
.mrd-pr-prompt-label{position:static;flex-shrink:0;margin:6px 0 2px 8px;font-size:9px;font-weight:bold;color:#666;text-transform:uppercase;letter-spacing:0.5px;pointer-events:none;user-select:none}
.mrd-pr-prompt-area{width:100%;height:auto;min-height:450px;background:transparent;color:#e0e0e0;border:none;padding:4px 8px 8px 8px;resize:none;font-size:12px;line-height:1.4;box-sizing:border-box;outline:none;display:block}
.mrd-pr-prompt-area:focus{border-color:#888}
.mrd-pr-audio-info{width:100%;height:100%;background:#181818;color:#aaa;border:1px solid #111;border-radius:6px;padding:10px;font-size:12px;line-height:1.6;box-sizing:border-box;display:none}
.mrd-pr-audio-info span{color:#fff;font-weight:500}
.mrd-pr-controls-group{background:#1e1e1e;border:1px solid #333;border-radius:6px;padding:6px 10px;display:flex;flex-direction:column;gap:4px;box-sizing:border-box;width:100%}
.mrd-pr-strength-row{display:flex;align-items:center;gap:12px;width:100%;box-sizing:border-box}
.mrd-pr-height-resizer{height:6px;background:#2a2a2a;cursor:ns-resize;border-radius:3px;margin:2px 0;transition:background 0.15s;border:1px solid #1e1e1e}
.mrd-pr-height-resizer:hover{background:#444;border-color:#555}
.mrd-pr-strength-label{font-size:11px;font-weight:600;color:#fff;white-space:nowrap;margin-left:auto;user-select:none;-webkit-user-select:none}
.mrd-pr-strength-slider{-webkit-appearance:none;appearance:none;width:80px;height:4px;background:#444;border-radius:2px;outline:none;cursor:pointer;border:1px solid #222}
.mrd-pr-strength-slider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:12px;height:12px;border-radius:50%;background:#aaa;cursor:pointer}
.mrd-pr-strength-slider:disabled{opacity:0.3;cursor:not-allowed}
.mrd-pr-strength-input{font-size:12px;color:#fff;background:#222;border:1px solid #444;border-radius:4px;width:52px;text-align:center;padding:3px;user-select:none;-webkit-user-select:none}
.mrd-pr-strength-input::-webkit-outer-spin-button,.mrd-pr-strength-input::-webkit-inner-spin-button{-webkit-appearance:none;margin:0}
.mrd-pr-strength-input[type=number]{-moz-appearance:textfield}
.mrd-pr-strength-input:disabled{opacity:0.35;cursor:not-allowed}
.mrd-pr-gap-menu{position:fixed;background:#1e1e1e;border:1px solid #444;border-radius:6px;padding:4px;display:flex;flex-direction:column;gap:4px;z-index:1000;box-shadow:0 4px 16px rgba(0,0,0,0.6)}
.mrd-pr-gap-menu-btn{background:#2a2a2a;color:#e0e0e0;border:1px solid #333;border-radius:4px;padding:6px 14px;font-size:11px;font-family:inherit;cursor:pointer;text-align:left;white-space:nowrap;display:flex;align-items:center;gap:6px;transition:background 0.15s ease}
.mrd-pr-gap-menu-btn:hover{background:#3a3a3a;border-color:#666}
.mrd-pr-player-controls{display:flex;justify-content:center;align-items:center;gap:12px;padding:2px 0;flex-wrap:wrap;width:100%}
.mrd-pr-icon-btn{background:#2a2a2a;border:1px solid #444;color:#eee;cursor:pointer;padding:6px 12px;border-radius:4px;display:flex;align-items:center;justify-content:center;transition:all 0.2s}
.mrd-pr-icon-btn *{pointer-events:none}
.mrd-pr-icon-btn:hover{color:#fff;background:#3a3a3a;border-color:#666}
.mrd-pr-icon-btn.active{color:#4fff8f;border-color:#4fff8f;background:#1a3a2a}
.mrd-pr-seek-bar{-webkit-appearance:none;appearance:none;height:6px;background:#444;border-radius:3px;outline:none;cursor:pointer;border:1px solid #222}
.mrd-pr-seek-bar::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:14px;height:14px;border-radius:50%;background:#ff4444;cursor:pointer;border:2px solid #222}
.mrd-pr-timeline-viewport{width:100%;overflow-x:auto;overflow-y:hidden;padding-bottom:10px;box-sizing:content-box}
.mrd-pr-timeline-viewport::-webkit-scrollbar{height:10px}
.mrd-pr-timeline-viewport::-webkit-scrollbar-track{background:#151515;border-radius:5px}
.mrd-pr-timeline-viewport::-webkit-scrollbar-thumb{background:#444;border-radius:5px;border:1px solid #000}
.mrd-pr-timeline-viewport::-webkit-scrollbar-thumb:hover{background:#666;border-color:#000}
.mrd-pr-zoom-controls{display:flex;align-items:center;gap:4px;margin-left:12px}
.mrd-pr-zoom-slider{width:80px;-webkit-appearance:none;appearance:none;height:4px;background:#444;border-radius:2px;outline:none;cursor:pointer}
.mrd-pr-zoom-slider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:12px;height:12px;border-radius:50%;background:#aaa;cursor:pointer}
.mrd-pr-right-group{display:flex;align-items:center;gap:12px}
.mrd-pr-segment-bounds{font-size:12px;color:#aaa;font-family:inherit;user-select:none;-webkit-user-select:none}
.mrd-pr-timecode{font-size:14px;font-weight:bold;color:#e0e0e0;font-family:inherit;user-select:none;-webkit-user-select:none}
.mrd-pr-settings-menu{position:fixed;background:#1e1e1e;border:1px solid #444;border-radius:6px;padding:10px;display:flex;flex-direction:column;gap:8px;z-index:1000;box-shadow:0 4px 20px rgba(0,0,0,0.7);min-width:250px}
.mrd-pr-settings-title{font-size:11px;font-weight:600;color:#888;text-transform:uppercase;letter-spacing:0.06em;padding-bottom:4px;border-bottom:1px solid #333;margin-bottom:2px}
.mrd-pr-settings-row{display:flex;align-items:center;justify-content:space-between;gap:8px}
.mrd-pr-settings-label{font-size:12px;color:#bbb;flex:1;white-space:nowrap}
.mrd-pr-number-control{display:flex;align-items:center;border:1px solid #444;border-radius:4px;background:#2a2a2a;overflow:hidden}
.mrd-pr-number-btn{background:#333;color:#aaa;border:none;width:20px;height:22px;cursor:pointer;font-size:12px;display:flex;align-items:center;justify-content:center;transition:background 0.15s;user-select:none}
.mrd-pr-number-btn:hover{background:#444;color:#fff}
.mrd-pr-settings-input{background:transparent;color:#e0e0e0;border:none;padding:0 4px;font-size:12px;width:50px;height:22px;text-align:center;font-family:inherit;outline:none;-moz-appearance:textfield}
.mrd-pr-settings-input::-webkit-outer-spin-button,.mrd-pr-settings-input::-webkit-inner-spin-button{-webkit-appearance:none;margin:0}
.mrd-pr-settings-select{appearance:none;-webkit-appearance:none;background:#2a2a2a url("data:image/svg+xml;charset=utf-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='10' height='10' viewBox='0 0 24 24' fill='none' stroke='%23999999' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpolyline points='6 9 12 15 18 9'/%3E%3C/svg%3E") no-repeat right 6px center;color:#e0e0e0;border:1px solid #444;border-radius:4px;padding:3px 22px 3px 6px;font-size:12px;width:132px;height:22px;box-sizing:border-box;cursor:pointer;outline:none;transition:border-color .15s ease}
.mrd-pr-settings-select:hover{border-color:#666}
.mrd-pr-settings-select:focus{border-color:#5c9dff}
.mrd-pr-settings-divider{border:none;border-top:1px solid #333;margin:4px 0}
.mrd-pr-settings-toggle-btn{width:100%;box-sizing:border-box;margin:0;background:#252525;color:#fff;border:1px solid #333;border-radius:4px;padding:5px 8px;font-size:11px;cursor:pointer;text-align:center;transition:all 0.15s}
.mrd-pr-settings-toggle-btn:hover{background:#2e2e2e;color:#fff;border-color:#555}
.mrd-pr-settings-close-btn{background:transparent;color:#888;border:none;cursor:pointer;padding:2px;display:flex;align-items:center;justify-content:center;border-radius:4px;transition:all 0.15s}
.mrd-pr-settings-close-btn:hover{color:#fff;background:rgba(255,255,255,0.1)}
.mrd-pr-segmented-control{display:flex;background:#1e1e1e;border:1px solid #333;border-radius:6px;padding:2px;width:110px;height:25px;align-items:center;box-sizing:border-box}
.mrd-pr-segment{flex:1;text-align:center;font-size:10px;font-weight:500;display:flex;align-items:center;justify-content:center;height:100%;cursor:pointer;border-radius:4px;color:#888;transition:all 0.15s ease}
.mrd-pr-segment.active{background:#333;color:#fff}
...
.tr-gp-input{background:#222;color:#e0e0e0;border:1px solid #3a3a3a;border-radius:4px;padding:3px 6px;font-size:11px;font-family:inherit;width:100%;box-sizing:border-box;outline:none;transition:border-color 0.15s ease}
...
.mrd-pr-out-mount{width:100%;box-sizing:border-box;flex-shrink:0}
...
.tr-out-input{background:#222;color:#e0e0e0;border:1px solid #3a3a3a;border-radius:4px;padding:3px 6px;font-size:11px;font-family:inherit;width:100%;box-sizing:border-box;outline:none;transition:border-color 0.15s ease}
